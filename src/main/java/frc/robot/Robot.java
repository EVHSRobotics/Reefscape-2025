...
public class Robot extends TimedRobot {
        ...
        public Robot() {
                ...
                QuestPublisher = NetworkTableInstance.getDefault().getStructTopic("Quest Pose", Pose2d.struct).publish();
        ...
        public void teleopPeriodic() {
                if (controller1.getXButton()) scheduler.cancelAll();
                ...
                if (controller.getPOV() ==0 ) container.mode = Container.Mode.Coral;
                if (controller.getPOV() == 180) container.mode = Container.Mode.Algae;

                if (controller.getRightTriggerAxis() >0.1){
                if (controller.getRightY() > 0.8) container.targetLow();
                if (controller.getRightX() > 0.8) container.targetMedium();
                if (controller.getRightY() < -0.8) container.targetHigh();
                if (controller.getRightX() < -0.8) container.targetTrough();
                }
        }
}
